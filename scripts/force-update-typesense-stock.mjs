...
/**
 * This script forces an update to the stock level of a variation in Typesense.
 * It bypasses the webhook and directly updates the Typesense document.
 * 
 * Usage: node scripts/force-update-typesense-stock.mjs <product_id> <variation_id> <stock_quantity>
 */
...
// Force update the stock level in Typesense
async function forceUpdateTypesenseStock() {
  try {
    console.log(`Force updating stock for product ${productId}, variation ${variationId} to ${stockQuantity} in Typesense...`);
    ...
    console.log(`✅ Successfully force updated variation in Typesense`);
    ...
    console.error('Error force updating Typesense stock:', error);
  }
}

// Run the update
forceUpdateTypesenseStock();
