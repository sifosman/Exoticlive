...
export async function generateMetadata({
  ...
  if (!product) {
    return {
      ...
      description: `Product not found on ${SITE_NAME}`
    };
  }

  // Return the metadata for the product
  return {
    ...
    openGraph: {
      title: `${product.name} | ${SITE_NAME}`,
      description: product.description || `${product.name} available at ${SITE_NAME}`,
      images: product.gallery_images && product.gallery_images.length > 0
        ? [{ url: product.gallery_images[0].url }]
        : []
    }
  };
}
